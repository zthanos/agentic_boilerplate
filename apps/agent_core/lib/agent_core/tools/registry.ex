defmodule AgentCore.Tools.Registry do
  @moduledoc """
  ...
  @type tool_item :: atom() | String.t() | Spec.t() | map()
  ...
  @known_by_id Map.new(@known_tools, fn %Spec{id: id} = spec -> {id, spec} end)
  ...
  """
  @spec fetch(tool_id()) :: {:ok, Spec.t()} | :error
    ...
    parsed =
      Enum.map(list, fn
        ...
        %{"id" => id} = m when is_binary(id) ->
          {:ok, Spec.new(id, name: m["name"], description: m["description"])}

        %{id: id} = m when is_binary(id) ->
          {:ok, Spec.new(id, name: Map.get(m, :name), description: Map.get(m, :description))}

        id when is_atom(id) or is_binary(id) ->
          {:ok, Spec.new(Spec.canonical_id(id))}
  ...
  defp resolve_id(%Spec{id: id} = spec) do
  ...
  defp dedup_keep_first(specs) do
    {acc, _seen} =
      Enum.reduce(specs, {[], MapSet.new()}, fn %Spec{id: id} = spec, {a, seen} ->
